...
thesis.pdf: $(thesis_config) $(thesis_files) $(thesis_figures) $(bibtex_file) template.latex
	@pandoc --toc --filter pandoc-shortcaption --filter pandoc-crossref --filter pandoc-citeproc --bibliography $(bibtex_file) --csl template/citationstyle.csl --template template.latex --latex-engine xelatex -o thesis.pdf $(thesis_config) $(thesis_files)
